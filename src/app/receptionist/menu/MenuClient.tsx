      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div className="space-y-4 rounded-2xl border bg-white p-4">
          ...
          <div className="space-y-2">
            {categories.map((cat) => (
              <div key={cat.id} className="flex items-center gap-2 rounded-xl border p-3">
                {editingCategoryId === cat.id ? (
                  ...
                  <button type="button" className="flex-1 text-left font-medium" onClick={() => setEditingCategoryId(cat.id)}>
                    {cat.name}
                  </button>
                )}
                <Button size="sm" variant="outline" onClick={() => setEditingCategoryId(cat.id)}>
                  Rename
                </Button>
        ...
        <div className="space-y-4 rounded-2xl border bg-white p-4">
          <h3 className="font-semibold">{form.id ? "Edit menu item" : "Add menu item"}</h3>
          <form onSubmit={form.id ? saveItem : addItem} className="grid gap-3 sm:grid-cols-2">
            ...
            <div className="sm:col-span-2 space-y-2 rounded-xl border border-stone-200 bg-stone-50 p-3">
              <label className="block text-sm font-medium text-stone-700">Menu image</label>
              <Input type="file" accept="image/*" onChange={(e) => { const file = e.target.files?.[0]; if (file) void uploadItemImage(file); }} />
              <p className="text-xs text-stone-500">{uploadingImage ? "Uploading image..." : "Upload a dish photo; it will be saved to the server automatically."}</p>
              {form.image && (
                <div className="relative h-32 overflow-hidden rounded-lg border bg-white">
                  <Image src={form.image} alt="Menu item preview" fill className="object-cover" />
                </div>
              )}
            </div>
            <select className="rounded-xl border border-stone-200 px-4 py-2.5 sm:col-span-2" value={form.categoryId} onChange={(e) => setForm({ ...form, categoryId: e.target.value })}>
              <option value="">No category</option>
              {categories.map((c) => (
                <option key={c.id} value={c.id}>{c.name}</option>
              ))}
            </select>
            <Button type="submit" className="sm:col-span-2">{form.id ? "Save changes" : "Add item"}</Button>
          </form>
        </div>
      </div>
        ...
        {items.map((item) => (
          <article key={item.id} className="rounded-xl border bg-white p-4">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <Button size="sm" variant="outline" onClick={() => setForm({ id: item.id, name: item.name, price: String(item.price), description: item.description || "", categoryId: item.categoryId || "", image: item.image || "" })}>
                Edit
              </Button>
              <Button size="sm" variant="outline" onClick={() => toggleAvailability(item.id, item.isAvailable)}>
